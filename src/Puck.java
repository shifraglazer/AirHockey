...
public class Puck {
	...
	public void move() {
		...
		// hit top/bottom walls
			...
			/*
			 * if (puckX > 50 && puckX < width - 50) { goal = true; reset(); }
			 * else {
			 */
			colorNum += .02;
			deltaY = -deltaY;
	...
	public void drawPuck(Graphics g) {
		...
		/*
		 * if (goal || !executor.isShutdown()) { g.setFont(new Font("Arial",
		 * Font.BOLD, 50)); g.setColor(Color.BLUE); g.drawString("GOAL!", 70,
		 * 260); System.out.println("goal!"); // time = 2; // executor =
		 * Executors.newScheduledThreadPool(1); //
		 * executor.scheduleAtFixedRate(timer, 0, 3, TimeUnit.SECONDS); goal =
		 * false; }
		 */
	...
	public void setSlope(int malletX, int malletY) {
		System.out.println("change slope");
